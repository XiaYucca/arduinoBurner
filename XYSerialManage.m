...
@property (nonatomic,copy)void(^readBlock)(NSData *data);
...
-(void)serialGATTCharValueUpdated:(NSString *)UUID value:(NSData *)data
{
  ...
  //  data getBytes:<#(nonnull void *)#> range:<#(NSRange)#>
    
    
    NSString *hexString = [NSString getHexStringWithData:data];
    
    NSLog(@"recive data length%lu, %@",data.length,hexString);
    
    !self.updteValue? : self.updteValue(self.serial.activePeripheral,data);
    
    !self.readBlock? : self.readBlock(data);
}
...
-(void)readDataWithBlock:(void(^)(NSData *Data))Block
{
